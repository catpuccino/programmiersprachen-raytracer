...
void SDFParser::parse_scene(std::string const& file_path, Scene& scene) {
    ...
    while (std::getline(in_sdf_file, line_buffer)) {
        ...
        if ("transform" == token_string) {
            ...
            auto shape = scene.shape_cont.find(object_name);
            ...
            if ("translate" == transform_type) {
                ...
                if (shape != scene.shape_cont.end()) {
                    shape->second->add_to_world_transformation(translate_matrix);
                }
                else {
                    return;
                }
            ...
            if ("rotate" == transform_type) {
                ...
                if (shape != scene.shape_cont.end()) {
                    shape->second->add_to_world_transformation(rotation_matrix);
                }
                else {
                    return;
                }
            ...
            if ("scale" == transform_type) {
                ...
                if (shape != scene.shape_cont.end()) {
                    shape->second->add_to_world_transformation(scale_matrix);
                }
                else {
                    return;
                }
